...
/// \returns stable IDs of available backends on the system.
...
///                   is populated with the number of backends supported.
...
ONNXIFI_PUBLIC ONNXIFI_CHECK_RESULT onnxStatus ONNXIFI_ABI onnxInitGraph(
    ...
    onnxGraph* graph) {
  if (onnxModel == NULL || weightDescriptors == NULL || graph == NULL) {
    return ONNXIFI_STATUS_INVALID_POINTER;
  }
  if (!onnxModelSize) {
